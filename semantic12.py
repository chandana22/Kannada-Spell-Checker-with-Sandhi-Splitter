...
def semantic(given_word,prnt_res):
	...
	(('F.PL'in tag_noun or 'MFN.PL' in tag_noun)and ('F.PL' in tag_verb or 'MFN.PL' in tag_verb))):
			...
			if ('P1' in tag_noun and ('P2' in tag_verb or 'P3' in tag_verb) and ('N.SL' in tag_verb or 'N.PL' in tag_verb)):
				to_the_file.append("ಈ ವಾಕ್ಯ ಉತ್ತಮ ಪುರುಷದಲ್ಲಿದೆ ಮತ್ತು ಇದರ ಕತೃ ಕ್ರಿಯಾಪದಗಳು ಹೊಂದಿಕೊಂಡಿವೆ")				                                
			
			elif ( 'P1' in tag_noun and 'P1' in tag_verb):
		...
		for letter in word:
			...
			if letter not in completion_dawg:
				...
				if suggest_word:
					...
					ret_list.append(0)
					ret_list.insert(1,"\nThe sentence has a spell error!")
					ret_list.insert(2, suggest_line)
					return ret_list
			nlines=0
			for line in lines:
	...
	(('F.PL'in tag_noun or 'MFN.PL' in tag_noun)and ('F.PL' in tag_verb or 'MFN.PL' in tag_verb))):
			...
			if ('P1' in tag_noun and ('P2' in tag_verb or 'P3' in tag_verb) and ('N.SL' in tag_verb or 'N.PL' in tag_verb)):
				to_the_file.append("The person is P1. \nThe sentence has subject-verb agreement")				 
			elif ( 'P1' in tag_noun and 'P1' in tag_verb):
								
				to_the_file.append("The person is P1. \nThe sentence has subject-verb agreement")
			elif ( 'P2' in tag_noun and 'P2' in tag_verb):
				
				to_the_file.append("The person is P2. \nThe sentence has subject-verb agreement")
			elif (( 'P3' in tag_noun and 'P3' in tag_verb) or ('P3' in tag_verb)):
				
				to_the_file.append("The person is P3. \nThe sentence has subject-verb agreement")
			
			else:
			
				to_the_file.append("Wrong person relationship. \nThe sentence has NO subject-verb agreement")
		elif found==0:
			#print("Error!!Gender or Tense or Person dont match!!\n")
			to_the_file.append("Error!!Gender doesn'nt match!!!!")
		elif found==1:
